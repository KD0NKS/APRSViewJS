...
function JavAPRSISConnection(args) {
    ...
    self.packetConverter = new ISPacketConverter();
	...
	if(args['keepAliveTime'] != null && args['keepAliveTime'] != undefined && args['keepAliveTime'] > 0) {
		...
		this.keepAlive = setInterval(this.SendLogin, this.keepAliveTime);
	}
...
function Send(message) {
    if(instance.isTransmitEnabled) {
        ...
        if(message instanceof APRSPositionReport) {
            console.log('Sending position packet: ' + message);

            instance.emit('position', rpt);
            instance.socket.write(msg, 'utf8');
