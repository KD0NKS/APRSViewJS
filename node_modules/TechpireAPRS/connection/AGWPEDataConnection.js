...
function AGWPEDataConnection(args) {
    ...
    // Stub to enable ability to connect but not monitor traffic.
    // The hardcoded value will change when somebody finds a reason for it to change.
    self.isMonitor = true;
...
AGWPEDataConnection.prototype._setEnabled = function(value) {
    var self = this;
    
    console.log('setting agwpe isenabled property');
            
    if(self._isEnabled != value) {
        self._isEnabled = value;

        // notify anybody listening the connection has been enabled/disabled
        self.emit('connectionChange', value);

        if(value == true) {
            console.log('enabling agwpe connection');
            self.Connect();
            ...
            console.log('disconnecting agwpe connection');
            self.Disconnect();
        }
    }
};
