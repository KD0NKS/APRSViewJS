...
AGWPEPacketParser.prototype.parse = function(packet) {
	if(packet.length > 35) {
		...
		var header = packet.slice(36).toString('utf8');
		var body = header.slice(header.indexOf('\r') + 1);
		header = header.slice(0, header.indexOf('\r'));
		...
		if(body.length > 0) {
			...
			} else if(dataType == 'd' || dataType == 'D' || dataType == 'S' || dataType == 'T'|| dataType == 'K') {
			...
			} else if(dataType == 'U') { // Monitored Unproto Information
				if(body.charAt(0) == '>') {
