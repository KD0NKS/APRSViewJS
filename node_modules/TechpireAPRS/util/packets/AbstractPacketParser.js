...
function AbstractPacketParser(args) {
	
}

// Extract the message type
AbstractPacketParser.prototype.extractMessageType = function(report, body) {
	report.messageType = body.charAt(0);
	return body.substring(1);
}

//Extract the latitude/longitude from the raw packet
AbstractPacketParser.prototype.extractPosition = function(report, body) {
	try {
...
AbstractPacketParser.prototype.extractMicE = function(report, body) {
...
AbstractPacketParser.prototype.extractTimeStamp = function(report, body) {
	var ts = new Date();
...
AbstractPacketParser.prototype.extractMessage = function(report, body) {
	if(body.indexOf("BLN") == 0) {
		report.addressee = "BLN";
		report.number = body[3].trim();
		report.addressee = body.substring(4, 5).trim()
		report.message = body.split(':')[1].trim();
	} else if(body.indexOf("NWS") == 0) {
		report.addressee = body.split(':')[0].trim();
		report.message = body.split(':')[1].trim();
	} else {
		report.addressee = body.split(':')[0].trim();
		...
		if(body.indexOf('{') > -1) {
			...
			report.message = msg.split('{')[0];
            
            report.number = msg.split('{')[1];
            report.number = report.number.trim();
            
			//report.MessageText = line.substring(10, (line.indexOf('{') - 11));
			//report.MessageId = line.substring(line.indexOf('{') + 1);
		} else {
            //console.log(body.split(':')[1].trim());
            
            try {
                report.message = body.split(':')[1].trim();
            } catch(e) {
                // TODO: SOMETHING IS WRONG HERE, but what?  Other than the obvious
                console.log(body);
            }
...
AbstractPacketParser.prototype.extractStatusReport = function(report, body) {
...
AbstractPacketParser.prototype.isIconRotatable = function(symbolTableId, symbolCode, speed, direction) {
...
AbstractPacketParser.prototype.InitializeMessages = function() {
...
AbstractPacketParser.prototype.InitializeMessages();

module.exports = AbstractPacketParser;